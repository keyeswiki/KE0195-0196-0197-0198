...
基于模块化硬件与本地语音识别技术，打造出"小智AI聊天机器人"交互套件。核心搭载ESP32-S3主控、MEMS数字麦克风（ICS-43432）、数字功放（NS4168）、1.54英寸LCD彩屏及扬声器，支持语音输入与实时播报，实现基础人机交互。标配电池盒与USB线缆，外接电源即开即用，装入电池后更可脱线随身携带。
...
| :--: | :--: | :--: | :--: |
| 组装半成品 | 即用型 ESP32 S3 Mini小智AI聊天机器人组装半成品 | 方便接线，固定主板及整体套装 | ![](media/image-20260918144659463.png) |
...
### 1. ESP32 S3 Mini开发板
![Img](media/MB0183.png)
KEYES ESP32 S3 Mini开发板是一款基于乐鑫ESP32-S3-WROOM-1-N16R8模组的高性能开发板，集成2.4 GHz Wi-Fi和低功耗蓝牙，搭载最高240 MHz双核Xtensa LX7处理器，内置16MB Flash、8MB PSRAM，支持机器学习硬件加速，可在语音识别、图像处理等AI场景下实现高效推理。
...
### 2. KEYES S3 AI Mini Sensor Shield扩展板

![Img](media/KS5035.png)


#### 简介  
KEYES ESP32 S3 AI Mini Sensor Shield是一款专为S3 Mini开发板设计的扩展板，将S3 Mini的大部分IO口以2.54 3pin排针的方式引出，方便连接各类外设模块。扩展板还集成了MIC接口、喇叭接口和屏幕接口，适配小智AI聊天机器人的外设连接。

#### 特点  
1. **统一接口**：所有传感器、功放、麦克风模块均通过排针整齐连接。    
2. **板载电源管理**：支持6~9V电池输入，通过MIC29302降压至5V，经AMS1117降压至3.3V，为各模块稳定供电。    
3. **专用接口**：提供麦克风接口、喇叭接口、屏幕接口，即插即用。  
4. **IO扩展**：引出多余GPIO，方便添加更多传感器。 

#### 主要参数

| 参数              | 数值 / 范围             | 说明                                        |
|-------------------|-------------------------|---------------------------------------------|
| **工作电压** | DC6-9V | 电池盒或直流电源输入 |
| **5V最大电流** | 1.5A | 为屏幕等外设供电 |
| **3.3V最大电流** | 0.5A | 为传感器等模块供电 |
| **排针间距** | 2.54mm | 标准间距 |
| **工作温度** | -25℃ ~ +55℃ | 适应多数常见环境 |
| **尺寸** | 64mm × 88mm | 适配亚克力固定板 |
...
![Img](media/KE0196.png)


2.  **连接设备**: 选择KE0196，将ESP32 S3 Mini开发板套装通过Type-C数据线连接到电脑。
3.  **网页连接设备**:
    -   在网页上点击“**连接设备**”按钮。
...
![](media/image-20260918144357238.png)
...
> **提示**：所有模块均通过扩展板统一接口连接，请确保各模块牢固插入对应接口。若问题依旧，确认固件版本与硬件配置一致。
