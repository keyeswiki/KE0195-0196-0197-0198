...
基于模块化硬件与本地语音识别技术，打造出"小智AI聊天机器人"交互套件。核心搭载ESP32-S3主控、MEMS数字麦克风（ICS-43432）、数字功放（NS4168）、1.54英寸LCD彩屏及扬声器，支持语音输入与实时播报，实现基础人机交互。开发板采用Arduino UNO标准引脚布局，并额外集成电流检测、SD卡挂载、USB-OTG及可编程RGB灯等实用功能，方便数据记录与外设扩展。标配电池盒与USB线缆，外接电源即开即用，装入电池后更可脱线随身携带。

## 主要特色
- **一板多能，进阶无忧**：板载电流检测、SD卡槽与可编程RGB灯，支持数据记录与状态指示，拓展更随心。
...
- **可扩展**：扩展板以2.54排针引出多余GPIO、I²C等接口，方便为机器人添加更多传感器或功能模块。  
...
| :--: | :--: | :--: | :--: |
|  组装半成品   | 即用型 ESP32 S3 Pro小智AI聊天机器人组装半成品 |    方便接线，固定主板及整体套装    | ![](media/image-20260918143322413.png) |
...
|     导线      |            5P双头杜邦母连线 100mm             |       模块与开发板之间的连接       |      ![Img](./media/19700349.png)      |


## 硬件介绍

### 1. ESP32 S3 PRO开发板
![Img](media/KS5034.png)

ESP32 S3 Pro开发板基于ESP32-S3-WROOM-1-N16R8模组设计，拥有16MB Flash，主频最高240MHz。开发板采用与Arduino UNO系列一致的引脚布局与外形尺寸，兼容标准扩展板，方便接入各类传感器与执行器模块。在核心性能之外，该开发板额外集成了电流检测、SD卡挂载、USB-OTG及板载可编程RGB灯等实用功能，并将绝大多数GPIO通过排针排母引出，板载DC-DC与LDO电源芯片可稳定输出5V/3.3V，满足常见外设的供电需求，适配AI语音交互、数据记录、便携设备等多种开发场景。板上同时预留复位按键（RST）与BOOT按键，方便调试与固件烧录。
...
- **板载外设与接口**
  - 板载可编程RGB灯（GPIO45），可用于状态指示
  - 板载INA180电流检测芯片（GPIO1），可实时监测系统功耗
  - 支持MicroSD/TF卡挂载（4线SDIO模式），便于数据存储与日志记录
  - 支持USB-OTG功能（GPIO19/20），可外接USB设备
  - 多数GPIO以排针排母引出，兼容Arduino UNO引脚布局，方便扩展

- **电源管理**
  - DC接口输入电压7~12V，整机主供电
  - USB Type-C接口提供5V供电及程序下载
  - 板载DC-DC降压输出5V/1.5A，LDO输出3.3V，满足外设供电需求
...
| --- | --- |
| GPIO **20** | **SCL**（SPI 时钟信号） |
| GPIO **21** | **SDA**（SPI 数据信号） |
| GPIO **38** | **RES**（复位信号） |
...
![Img](media/KS5029_02.png)

![Img](media/KS5029_03.png)

![Img](media/KS5029_04.png)

![Img](media/KS5029_05.png)
...
2.  **连接设备**: 选择KE0197，将ESP32 S3 Pro开发板套装通过Type-C数据线连接到电脑。
3.  **网页连接设备**:
    -   在网页上点击“**连接设备**”按钮。
...
![](media/image-20260918144331489.png)
...
> **提示**：所有模块均通过扩主板统一接口连接，请确保各模块牢固插入对应接口。若问题依旧，确认固件版本与硬件配置一致。
