# KE0195 即用型 ESP32 S3 Cam小智AI聊天机器人DIY套装 摄像头版本 LCD1.54彩屏

![Img](media/KE0195.jpg)

## 简介

基于模块化硬件与本地语音识别技术，打造出"小智AI聊天机器人"交互套件。核心搭载ESP32-S3主控、摄像头、MEMS数字麦克风（ICS-43432）、数字功放（NS4168）、1.54英寸LCD彩屏及扬声器，支持语音输入与实时播报，新增视觉识别能力，实现更丰富的AI交互体验。标配电池盒与USB线缆，外接电源即开即用，装入电池后更可随身携带。
...
| 硬件名称 | 规格/型号 | 主要用途 | 图片 |
| --- | --- | ---- | --- |
| 开发板 | ESP32 S3 Cam 开发板 | 主控板，负责运行固件、处理音视频与网络连接 | ![Img](media/MB0200.png) |
| 显示屏        | 1.54寸TFT屏 ST7789驱动 240×240                | 显示状态、对话信息等                       | ![Img](./media/LCD.png)                |
| 组装半成品    | 即用型 ESP32 S3 Cam小智AI聊天机器人组装半成品 | 方便接线，固定主板及整体套装                  | ![](media/image-20260918135147433.png) |
...
-   **摄像头接口**：板载24-pin摄像头连接器，通常搭配摄像头使用。
...
### 2. ESP32 S3 Cam AI Sensor Shield 扩展板

![Img](./media/MD2001.png)

#### 简介
ESP32 S3 Cam AI Sensor Shield 是基于 ESP32-S3-CAM 开发的扩展板，通过2.54mm排针接口扩展S3-CAM开发板的引脚，并引出了I²C、I²S、串口、1.54寸彩屏等接口。通过DC接口引入直流6~9V电源即可供电，拨动开关便于控制电源通断。四个M4定位孔，兼容乐高积木件大小。
...
- **军工级标准**: 具备军工级工艺标准，确保长期稳定工作，适用于各种环境。
...
## 麦克风接口
| **ESP32S3-CAM开发板** | **麦克风 ICS-43432（I2S接口）** |
| --- | --- |
| GPIO **1** | **WS** (数据选择) |
| GPIO **2** | **SCK** (数据时钟) |
| GPIO **42** | **DIN** (数据信号) |
...
| **ESP32S3-CAM开发板** | **1.54寸 IPS 彩屏 LCD** |
| --- | --- |
| GPIO **19** | **SCL**（SPI 时钟信号） |
| GPIO **20** | **SDA**（SPI 数据信号） |
| GPIO **21** | **RES**（复位信号） |
| GPIO **47** | **DC**（数据/命令选择信号） |
| GPIO **45** | **CS**（片选信号） |
| GPIO **38** | **BLK**（背光控制信号，高电平点亮） |
| **5V** | **VCC**（电源正 5V） |
| **GND** | **GND**（接地） |
...
2.  **连接设备**: 选择KE0195，将ESP32 S3 CAM开发板套装通过Type-C数据线连接到电脑,首次使用需安装CH340驱动，网页左上角点击“**下载安装驱动**”。
...
## ![](media/image-20260918144233519.png)
